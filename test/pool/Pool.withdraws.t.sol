...
contract PoolWithdrawsTest is BaseTest{
    ...
    function test_basicWithdraw() public{
        ...
        uint liquidity = utils.depositAmounts(pool, token0Amount, token1Amount, lp);
        (address _token0, address _token1) = pool.getTokens();
        (ERC20 token0, ERC20 token1) = (ERC20(_token0), ERC20(_token1));
        uint balance0 = liquidity * token0.balanceOf(address(pool)) / pool.totalSupply();
        uint balance1 = liquidity * token1.balanceOf(address(pool)) / pool.totalSupply();
        (uint withdrewAmount0, uint withdrewAmount1) = utils.withdrawAmounts(pool, liquidity, lp);
